#!/bin/bash
# Script para construir y ejecutar el sistema de orquestación
...
echo "¿Desea hacer ping inicial a los workers? (y/n)"
read -r response
if [[ "$response" == "y" || "$response" == "Y" ]]; then
    echo "Haciendo ping a workers..."
    sleep 5
    curl -s http://localhost:5000/ping_all | python -m json.tool
fi
